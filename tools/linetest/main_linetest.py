...
# Gate with a flag file instead of sys.exit(): a SystemExit in main.py makes the
# VM soft-reboot, after which machine.reset_cause() reports 5 (SOFT) and the
# real reason for the previous chip reset is lost.
RUN = 'run_once' in os.listdir('/')
if RUN:
    os.remove('/run_once')
